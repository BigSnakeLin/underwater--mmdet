...
test_cfg = dict(
    rpn=dict(
        ...
        min_bbox_size=0),
    rcnn=dict(
        score_thr=0.001, nms=dict(type='soft_nms', iou_thr=0.5, min_score=0.0001), max_per_img=200))
# dataset settings
...
lr_config = dict(
    ...
    warmup_ratio=1.0 / 3,
    step=[12, 15])
checkpoint_config = dict(interval=1, create_symlink=False, save_optimizer=False)
# yapf:disable
log_config = dict(
    interval=50,
    hooks=[
...
log_level = 'INFO'
work_dir = '/content/drive/My Drive/UnderWaterDetect/cascade_x101_32x4d_fpn_16e'
